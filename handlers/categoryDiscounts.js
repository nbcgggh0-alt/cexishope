...
async function handleCategoryDiscountDetail(ctx, categoryId) {
  ...
  const text = lang === 'ms'
    ? `🏷️ *Diskaun: ${category.icon} ${category.name.ms}*\n\n` +
      ...
      `\n\nPilih tindakan:`
    : `🏷️ *Discount: ${category.icon} ${category.name.en || category.name.ms}*\n\n` +
  ...
  await ctx.reply(
    lang === 'ms'
      ? `✅ *Diskaun berjaya ditambah!*\n\n` +
        `🏷️ Kategori: ${categories[categoryIdx].name.ms}\n` +
        `💰 Diskaun: ${discountText}\n\n` +
        `Semua produk dalam kategori ini kini mendapat diskaun ini.`
      : `✅ *Discount successfully added!*\n\n` +
        `🏷️ Category: ${categories[categoryIdx].name.en || categories[categoryIdx].name.ms}\n` +
        `💰 Discount: ${discountText}\n\n` +
